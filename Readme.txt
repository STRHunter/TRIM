...
We have used the distance-based phylogney. There major two steps in computation of distance-based phylogney 
...
i) What is does?
   ...
   Compute TRIM distance matrix from a set of TRIM vectors. The TRIM vectors should be generated by using the utility 'TRIM'
...
b) Contruction of phylogeny tree using hierarchical clustering algorithm like UPGM or NJ from the distance matrix.

  In this step we used MATLAB to costruct the phylogeny tree from the distance matric stored in 'TRIM_distance_matrix.txt'.
  The following script will load the TRIM distance matrix from file 'TRIM_distance_matrix.txt' to a MATLAB variable 'D'
