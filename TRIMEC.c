...
int main (int argc, char *argv[]) 
{
  ...
  unsigned long i,j,k=0; char filename[1000];
  ...
  for (i=6;i<=7; i++)
  {
   ...
   printf("at k=%lu",i); //if (i==6) cut_off=12; else if (i == 7) cut_off=14; 
 ...
 // TRIM vector of the input genome at k=6 is stored in TRIM[0][0-4095]
 // TRIM vector of the input genome at k=7 is stored in TRIM[1][0-16383]
 if (k==7)
{
  FILE *fp;
   ...
   for (j=0;j<pow(4,k);j++)
    {
       ...
       TRIM_Vector[i][j] = 0; // Canceling features that are not in feature[] 
    }
 } 
...
else if (k==6)
{
 ...
 for (i=0;i<1;i++) 
 {
  ...
  if (k==6)
  printf("Locading corresponding classifier at k=%lu....TRIMEC-6\n",k);
  else if (k==7)
  printf("Locading corresponding classifier at k=%lu....TRIMEC-7\n",k);
 ...
 double **Mean_TRIM_Non_animal;
  ...
  if (k==7) // loading the classifiers under TRIMEC-7 into memory
 {
   for (i =0;i<25;i++)
   {
    //printf("\nInstance %d ",i);
   ...
   }// end for 25 classifiers
  }// end if k=7
   
 else if (k ==6) // loading the classifiers TRIMEC-6 into memory
 {
 ...
 if (k ==6)
 {
  ...
  // Declaring the taxa 
  if (k==6)
  printf("\n\nBased on the maximum agreement among the 25 classifier under TRIMEC-6\n");
  else if (k==7)
    printf("\n\nBased on the maximum agreement among the 25 classifier under TRIMEC-7\n");
...
void TRIM(int k, char *fileName)
{
   ...
   fseek(fp, 0, SEEK_END);
   len = ftell(fp);   
   dna = (unsigned char *) malloc(sizeof(unsigned char)*len +1);
   fseek(fp, 0, SEEK_SET);// going begin of the file   
   fread(dna, len , 1, fp);   
   dna_len = strlen(dna); 
   fclose(fp);
    ...
    {    
      ...
      for (j =0;j<5000;j++) // computing entropy
       ...
       if (entropy > 0.0)
        {
          //printf("%.14lf\n", entropy/length); 
          TRIM_Vector[k-6][i] = entropy/length;      
        }
      else 
        {
           //printf("0\n");
